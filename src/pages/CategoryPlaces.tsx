...
export default function CategoryPlaces() {
  const { cityId, category } = useParams<{ cityId: string; category: string }>();
  ...
  const filteredPlaces = useMemo(() => {
    let items = places.filter(p => p.cityId === cityId && p.category === cat);
    ...
    return items;
  }, [places, cityId, cat, sortMode, filterMode]);
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => copyName(place.name)} className="flex-shrink-0 text-muted-foreground hover:text-primary transition-colors" title="Copiar nombre">
                    <Copy className="h-3.5 w-3.5" />
                  </button>
                </div>
                {place.altName && <p className="text-xs text-muted-foreground">{place.altName}</p>}
                <div className="flex flex-wrap gap-1 mt-1">
                  {place.status === 'must' && (
                    <span className="text-[9px] font-bold bg-travel-important-bg text-travel-important px-1.5 py-0.5 rounded-full">Imprescindible</span>
                  )}
                  {place.status === 'visited' && (
                    <span className="text-[9px] font-bold bg-travel-confirmed-bg text-travel-confirmed px-1.5 py-0.5 rounded-full">✓ Visitado</span>
                  )}
                  {place.tags.map(tag => (
                    <span key={tag} className="text-[9px] bg-muted text-muted-foreground px-1.5 py-0.5 rounded-full">{tag}</span>
                  ))}
                </div>
                {place.notes && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{place.notes}</p>}
              </div>
            </div>
